...
const App: React.FC = () => {
  const [elements, setElements] = useState<
    { type: string, content?: string, twitterLink?: string, emailLink?: string, links?: string[] }[]
  >([]);
  ...
  const updateLinks = (index: number, links: string[]) => {
    const newElements = [...elements];
        ...
        <VStack spacing={4} align="stretch">
          ...
          {elements.map((element, index) => (
            <Box key={index} p={4} borderWidth="1px" borderRadius="lg">
              ...
              {element.type === 'links' && <Links index={index} links={element.links} updateLinks={updateLinks} />}
              ...
              {element.type === 'partnerLogos' && <PartnerLogos index={index} links={element.links} updateLinks={updateLinks} />}
            </Box>
          ))}
